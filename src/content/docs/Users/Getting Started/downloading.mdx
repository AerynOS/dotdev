...
2. Look for the latest release available for download, the official ISO files are named `AerynOS-<version>-<desktop>-<architecture>.iso`.
  ...
  <Aside>
  Their may be multiple versions available with different desktop environments denoted by `AerynOS-<version>-<desktop>-<architecture>.iso` where `<desktop>` is the desktop environment.
  </Aside>
