...
This will make it so you can view `diff` output for binary `manifest.*.bin` files in both `git diff` and `git log -p .` invocations.
...
The recipe repo already contains the `.gitattribute` file that sets up the `moss` diff filter referenced here.
